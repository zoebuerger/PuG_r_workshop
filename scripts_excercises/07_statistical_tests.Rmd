...
t.test(nerd_fm$nerd_score ~ nerd_fm$gender)
#this standard R t-test doesn't assume equal variances 
```
...
nerd_fm %>% 
  group_by(gender) %>% 
  summarise(mean_nerd = mean (nerd_score),
            sd_nerd = sd(nerd_score)
            )
```
...
t.test(nerd_fm$Q1, nerd_fm$Q2, paired = TRUE)
...
nerd_fm$nerd_score[nerd_fm$gender == "male"])
...
```{r}
# run a one-sided t-test with mu = 104
...
3.2) The function `cohens_d()` from the package follows exactly the same logic as the `t.test()` function. Can you run an independent and a paired t-test, and then calculate the corresponding effect size?

```{r}
cohens_d(nerd_fm$nerd_score ~ nerd_fm$gender)
```


3.3) Extract the effect size and save the number into a variable.
```{r}
effectsize_gender <- cohens_d(nerd_fm$nerd_score ~ nerd_fm$gender)
effectsize_gender$Cohens_d
```
...
```{r}
cor.test(nerd_fm$age, nerd_fm$nerd_score)
```
