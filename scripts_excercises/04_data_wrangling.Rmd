...
vector_with_NAs %>% abs() %>% head(4) %>% mean(na.rm = TRUE) %>% as.character()
#OR 
as.character(mean(head(abs(vector_with_NAs),4), na.rm = TRUE))
#good practice representation(everything gets shown in a separate line):
vector_with_NAs %>% 
  abs() %>% 
  head(4) %>% 
  mean(na.rm = TRUE) %>% 
...
#here we can see that there is one coded 0 value, which is an error!
```
```{r}
...
```
```{r}
#always remember to ungroup your table when done!
nerd_red %>% 
  ungroup()
```
...
#with pull, we extract age as a vector, with select, it gives us a table with one column 
```
...
nerd_red <- 
  nerd_red %>% 
    select (!age_character, everything())
...
nerd_red %>% 
  ...
  ungroup()
    #you should not forget to ungroup your data, because it will still operate rowwise in the next function if you don't ungroup!!
...
ifelse(some_numbers > 9, "more than one digit", "one digit")
```
