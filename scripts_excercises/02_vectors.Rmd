...
```{r}
weird_combo <-  c(2, 4, 6, 5) + c(1, 10)
weird_combo
#this repeats the second vector to add to the second part of the first vector. but only if one vector is a multiple of the other
```
...
```{r}
not_fibonacci <- c(2,4,6,4,9,12) * c(1,2)
not_fibonacci
c(2,4,6,4,9,12) * 1:2
```
...
```{r}
almost_counting <- c(2,4,6,4,9,12) - c(1,2,3)
almost_counting
c(2,4,6,4,9,12) - 1:3
```
...
```{r}
hotdog_genders <- 
  c("female", "male", "male", "male", "male", "female", "female", "female", "male", "female",
    "male")
```
...
head(hotdog_2019)
tail(hotdog_2019)
#gives the first / last 6 elements of the vector
```
...
head(hotdog_names)
#the second argument chooses how many elements you want displayed
...
```{r}
#If there are lee in the vector than the second number, it'll just show everything
...
head(2,hotdog_names) #this does not work because R does not know which is which
...
head(n=2, x= hotdog_names) #if you define the elements R can compute it
```
...
c(hotdog_names[3], hotdog_names[5]) #not convention, longer
hotdog_names[c(3,5)]
...
```{r}
(hotdogs_2019_men <- hotdog_2019 [hotdog_genders == "male"])
(hotdogs_2018_women <- hotdog_2018 [hotdog_genders == "female"])
```
...
female_results <- hotdog_2019 [hotdog_genders=="female"]
female_names [female_results == max(female_results)] 
#THIS here is not possible, because the max of hotdog_2019 is a man, not a woman!
(hotdog_names [hotdog_genders=="female"]) [(hotdog_2019 [hotdog_genders=="female"]) == max(hotdog_2019)]
...
5.1) `sd()`
```{r}
sd(c(1, 2, NA, 4, 3, 6))#does not work
sd(c(1, 2, NA, 4, 3, 6), na.rm = TRUE)
...
5.2) `min()` and `max()`
```{r}
min(c(1, 2, NA, 4, 3, 6))#does not work
min(c(1, 2, NA, 4, 3, 6), na.rm = TRUE)

max(c(1, 2, NA, 4, 3, 6))#does not work
max(c(1, 2, NA, 4, 3, 6), na.rm = TRUE)
?max
```
...
5.3) `summary()`
```{r}
summary(c(1, 2, NA, 4, 3, 6))
summary(c(1, 2, NA, 4, 3, 6), na.rm = TRUE) #both give same info
```
...
```{r}
table(c(1, 2, NA, 4, 3, 6))
table(c(1, 2, NA, 4, 3, 6), na.rm = TRUE) #this does not work
?table
```
...
```{r}
head(c(1, 2, NA, 4, 3, 6))
head(c(1, 2, NA, 4, 3, 6), na.rm = TRUE) #both work

tail(c(1, 2, NA, 4, 3, 6))
tail(c(1, 2, NA, 4, 3, 6), na.rm = TRUE) #both work
```
...
```{r}
length(c(1, 2, NA, 4, 3, 6))
length(c(1, 2, NA, 4, 3, 6), na.rm = TRUE)
```
...
```{r}
TRUE & NA #gives NA because doesn't know what NA is, could be FALSE or TRUE
FALSE & NA # FALSE because the first thing is FALSE, so it always will be FALSE no matter what NA would be
TRUE | NA #here it will always be TRUE because of the first
FALSE | NA # here same as second one 
```
